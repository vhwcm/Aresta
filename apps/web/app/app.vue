<template>
  <div
    class="min-h-screen text-textPrimary selection:bg-accent/20 transition-all"
    :class="[
      isPanelBgPage ? 'bg-bgPanel' : 'bg-bgApp',
...
const isPanelBgPage = computed(() => {
  const currentPath = route?.path || ''
  return currentPath === '/beneficios'
})
