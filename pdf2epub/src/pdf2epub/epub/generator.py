...
class EpubGenerator:
    ...
    def generate(self, document: Document, output_path: str | Path) -> Path:
        ...
        book.set_language(document.metadata.language)
        ...
        # 3. Adiciona capítulos
        epub_chapters: List[epub.EpubHtml] = []
        toc_entries = []

        for idx, chap in enumerate(document.chapters):
            chap_file = f"text/chap_{idx+1:03d}.xhtml"
        ...
        # Spine com ordem dos itens de leitura
    ...
    def _render_chapter_html(self, chapter: Chapter) -> str:
        ...
        for r in chapter.regions:
            if r.type == RegionType.TITLE:
                html_parts.append(f'<h1>{escape_text(r.text)}</h1>')
                is_first_p_after_heading = True
            elif r.type == RegionType.HEADING:
                level = min(max(r.level, 2), 6)
                html_parts.append(f'<h{level}>{escape_text(r.text)}</h{level}>')
                is_first_p_after_heading = True
            elif r.type == RegionType.PARAGRAPH:
                cls_attr = ' class="first-after-heading"' if is_first_p_after_heading else ''
                html_parts.append(f'<p{cls_attr}>{escape_text(r.text)}</p>')
                is_first_p_after_heading = False
            elif r.type == RegionType.QUOTE:
                html_parts.append(f'<blockquote><p>{escape_text(r.text)}</p></blockquote>')
                is_first_p_after_heading = False
            elif r.type == RegionType.FOOTNOTE:
                html_parts.append(f'<aside class="footnote" epub:type="footnote"><p>{escape_text(r.text)}</p></aside>')
                ...
                html_parts.append(f'<figcaption>{escape_text(r.text)}</figcaption>')
