...
1. CRIAÇÃO DE ANOTAÇÃO E VETORIZAÇÃO (RUNTIME / ASYNC)
────────────────────────────────────────────────────────────────────────────────
  ...
  [ AnnotationController ]
         │
         ▼
  [ AnnotationService ] ──► [ AIClient (gRPC) ] ──► [ Go Gemini Microservice ]
         │                                                   │ (text-embedding-004)
...
────────────────────────────────────────────────────────────────────────────────
  ...
  [ FlashcardSchedulerService ]
         │
         ├─► Busca Anotações do Usuário onde (flashcard IS NULL)
         │
         ...
         │              ▼
         │          [ Gemini Pro / Flash AI ]
         │          (Prompt especializado com Small-Shot:
         │           1. Situação Real
         │           2. Relembração de Conceito
         │           3. União de Conceitos)
         │              │
         │              ▼
...
3. PREPARAÇÃO DO DECK DIÁRIO DE 50 CARDS (JOB 00:00 / SOB DEMANDA)
────────────────────────────────────────────────────────────────────────────────
  ...
  [ DailyDeckService ]
         │
         ├─► Busca até 50 Flashcards do Usuário:
         │     1. Prioridade A: Vencidos / Hoje (next_review_at <= hoje)
...
4. CONSUMO NO FRONTEND & INTEGRAÇÃO COM OFENSIVA (STREAK)
────────────────────────────────────────────────────────────────────────────────
  ...
  POST /api/flashcards/:id/review (rating: 'hard' | 'good' | 'easy')
         ...
         └─► Invoca StreakService.recordActivity(user_id, { flashcards_reviewed: 1 })
                 │
                 ▼
